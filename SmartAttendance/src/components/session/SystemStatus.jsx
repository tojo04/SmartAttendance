...
const VisualQRShare = () => {
  ...
  useEffect(() => {
    if (showStaticQR && !qrCodeDataURL) {
      generateQRCode();
    }
  }, [showStaticQR, qrCodeDataURL]);

  const toggleQRMode = () => {
    console.log('Toggling QR mode, current showStaticQR:', showStaticQR);
    setShowStaticQR(!showStaticQR);
    
    // Generate QR code when switching to static mode
    if (!showStaticQR) {
      console.log('Switching to static mode, generating QR code...');
      generateQRCode();
    }
  };
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      // Generate QR code with attendance data (independent of server)
      ...
      // Try to send session data to server (optional - won't block QR generation)
      try {
        await axios.post('http://localhost:3001/api/sessions/create', {
          sessionId: attendanceData.sessionId,
        });
        console.log('Session created on server');
      } catch (serverError) {
        console.warn('Server not available, QR generated locally:', serverError.message);
      }
            ...
            {qrCodeDataURL ? (
              <div className="text-center">
                <img 
                  ...
                  className="w-48 h-48 mx-auto rounded-lg shadow-lg border-2 "
                />
